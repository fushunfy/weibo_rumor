...
# +表示匹配到的多个字符, 匹配前面的子表达式一次或多次。要匹配 + 字符，请使用
...
for i in range(5):
    ...
    train_lines, train_labels = train['text'].values, train['label'].astype(int).values
    ...
    """
    model3.fit(train_x, train_y, validation_split=0.0, validation_data=None,
               callbacks=[early_stopping], initial_epoch=nb_epochs, batch_size=bsz, epochs=nb_epochs * 2,
               shuffle=True)
